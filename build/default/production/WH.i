...
volatile static uint8_t WH_EEPROMPassword_G[8] = {'#','P','w','r','U','p','_','1'};
volatile static uint8_t WH_PasswordRead_G[8] = {0};
...
void WH_Init(void)
{
    ...
    EEPROM_ReadPage(0, WH_PasswordRead_G, 8);
    ...
    {
        WH_TargetTemp_G = EEPROM_Read(8);
    }else
    {
        EEPROM_WritePage(0, WH_EEPROMPassword_G, 8);
        SW_Delay_ms(10);
        EEPROM_Write(8, WH_TargetTemp_G);
    }
}
...
void WH_NormalMode(void)
{
    ...
    SSD_SetSymbol((5), WH_AvgTemp_G%10);
    SSD_SetSymbol((4), (WH_AvgTemp_G/10)%10);
    if(SW_GetState((2)) || SW_GetState((1)))
    {
        WH_State_G = TempSetMode;
    }else if((SW_GetState((0))))
    {
...
void WH_TempSetMode(void)
{
    WH_PreState_G = WH_State_G;
    DISP_SetState(3, 1);
    SSD_SetSymbol((5), WH_TargetTemp_G%10);
    SSD_SetSymbol((4), (WH_TargetTemp_G/10)%10);
    if(SW_GetState((2)) && WH_TargetTemp_G <= 70)
    {
        WH_TargetTemp_G += 5;
        WH_Counter_G = 0;
    }
    if(SW_GetState((1)) && WH_TargetTemp_G >= 40)
    {
        WH_TargetTemp_G -= 5;
        WH_Counter_G = 0;
    }
